...
export default function PaymentTracking() {
  ...
  const filteredPayments = payments.filter((payment) => {
    const matchesSearch =
      (payment.project?.toLowerCase() || "").includes(
        searchTerm.toLowerCase()
      ) ||
      (payment.title?.toLowerCase() || "").includes(searchTerm.toLowerCase());

    if (activeFilter === "all") return matchesSearch;
    if (activeFilter === "validé")
      return matchesSearch && payment.status === "validé";
    if (activeFilter === "en_attente")
      return matchesSearch && payment.status === "en_attente";
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="text-white">
              ...
              <p className="text-amber-100 mt-1">
                Gérez les paiements associés à vos projets
              </p>
            </div>
          </div>
        </div>
        ...
        {filteredPayments.length > 0 ? (
          <>
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-100/50 overflow-x-auto mb-4">
              ...
              <div className="hidden sm:grid grid-cols-12 bg-gray-50/50 p-4 border-b border-gray-100/50 min-w-[700px]">
                <div className="col-span-3 font-medium text-sm text-gray-500">
                  Projet
                </div>
                <div className="col-span-2 font-medium text-sm text-gray-500">
                  Montant
                </div>
                <div className="col-span-2 font-medium text-sm text-gray-500">
                  Méthode
              ...
              {currentPayments.map((payment) => (
                ...
                >
                  ...
                  <div className="sm:col-span-3">
                    ...
                    <div className="text-xs text-[#dd7109] mt-1">
                      {payment.title}
                    </div>
                  </div>
                  {/* Montant */}
                  <div className="sm:col-span-2">
                    <span className="block sm:hidden text-xs text-gray-500 mb-0.5">
                      Montant
                    </span>
                  ...
                  <DropdownMenu.Root>
                    <DropdownMenu.Trigger asChild>
                      <button
                        className="p-2 rounded-full hover:bg-[#dd7109]/10 transition-colors"
                        aria-label="Menu paiement"
                      >
                        <svg
                          width="20"
                          height="20"
                          ...
                          viewBox="0 0 20 20"
                        >
                          <circle cx="4" cy="10" r="1.5" fill="#dd7109" />
                          <circle cx="10" cy="10" r="1.5" fill="#dd7109" />
                          <circle cx="16" cy="10" r="1.5" fill="#dd7109" />
                        </svg>
                      </button>
                    ...
                    <DropdownMenu.Portal>
                      <DropdownMenu.Content
                        align="end"
                        className="z-50 min-w-[150px] rounded-xl bg-white shadow-xl border border-gray-100 py-2"
                      >
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-12 text-center border border-gray-100/50">
            <div className="mx-auto max-w-md">
              ...
              <h3 className="mt-2 text-lg font-medium text-[#dd7109]">
                Aucun paiement trouvé
              </h3>
              <p className="mt-1 text-gray-500">
                Essayez de modifier vos critères de recherche ou enregistrez un
                nouveau paiement.
              </p>
            </div>
          </div>
