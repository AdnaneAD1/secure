...
async function updatePaymentStatus(paymentLinkId: string, status: string) {
  try {
    ...
    if (!paymentsSnapshot.empty) {
      ...
      const updateData = {
        dateValidation: new Date().toISOString().split("T")[0], // Format YYYY-MM-DD
      };

      // Si le paiement est validé, on ajoute la date de validation
      if (newStatus === "validé") {
        updateData.dateValidation = new Date().toISOString().split("T")[0]; // Format YYYY-MM-DD
      }
